...
function Ensure-DestinationFileNameIndex {
    ...
    # IMPORTANT: Every return path must use Write-Output -NoEnumerate.
    # HashSet[string] implements IEnumerable<string>, so a plain 'return'
    # lets PowerShell enumerate the set through the pipeline:
    #   - empty HashSet  -> $null   (nothing emitted)
    #   - 1-item HashSet -> string  (single element)
    #   - N-item HashSet -> array   (individual strings)
    # -NoEnumerate preserves the HashSet as a single scalar object.

    if ($IndexCandidate -is [System.Collections.Generic.HashSet[string]]) {
        Write-Output -NoEnumerate $IndexCandidate
        return
    }
    ...
    Write-Output -NoEnumerate $emptySet
}
